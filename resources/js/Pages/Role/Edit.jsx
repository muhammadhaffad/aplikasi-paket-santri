...
export default function Edit({ auth, data }) {
    ...
    const [permissionSelected, setPermissionSelected] = useState(data.permissions.map((permission) => ({ id: permission.id, label: permission.nama })));
    useEffect(() => {
        setData('permissions', permissionSelected.map((permission) => permission.id));
    }, [permissionSelected]);
    const submit = (e) => {
        e.preventDefault();
        put(route('access-control.role.update', { id: data.id }), {
            preserveScroll: true,
            preserveState: true,
            onError: (e) => {
                if (e.error) {
    ...
    return <AuthenticatedLayout
        ...
        header={
            <div className="flex items-center gap-4">
                ...
                <h2 className="font-semibold text-xl text-gray-800 leading-tight">Ubah Role</h2>
            </div>
        }
    >
