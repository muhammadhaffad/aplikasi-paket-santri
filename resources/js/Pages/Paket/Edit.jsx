...
export default function Edit({ auth, asramas, jenis_paket, paketKategoris, paket }) {
    const { data, setData, put, errors, processing, reset, recentlySuccessful, isDirty, setDefaults } = useForm({
        santri_nis: paket.santri_nis,
        nama: paket.nama,
        kategori_id: paket.kategori_id.toString(),
        pengirim: paket.pengirim,
        penerima: paket.penerima,
        tanggal_diterima: paket.tanggal_diterima,
        keterangan_disita: paket.keterangan_disita,
        asrama_id: paket.asrama_id.toString(),
        status: paket.status,
        jenis_paket: paket.jenis_paket,
    });
    const submit = (e) => {
        e.preventDefault();
        put(route('pakets.update', { jenis_paket: jenis_paket, id: paket.id }), {
            preserveScroll: true,
            preserveState: true,
            onError: (e) => {
                if (e.error) {
    ...
    return <AuthenticatedLayout
        ...
        header={
            <div className="flex items-center gap-4">
                ...
                <h2 className="font-semibold text-xl text-gray-800 leading-tight">Ubah Paket {jenis_paket === 'masuk' ? 'Masuk' : 'Keluar'}</h2>
            </div>
        }
    >
        <Head title={`Ubah Paket ${jenis_paket === 'masuk' ? 'Masuk' : 'Keluar'}`} />
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <Autocomplete
                                            ...
                                            onChange={(e) => setData('santri_nis', e.target.value)}
                                            onSelect={(value) => {
